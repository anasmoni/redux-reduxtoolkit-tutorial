...
**Prerequisites:** JS spread syntex, object destrucuring
...
- for managing medium/large amount of states globally
  in your app. Redux store holds state. Redux tooklit provider passes store to react components and its childs so that all the components can access states.
...
- dispatch an Action. (aka call reducer with action)
- Reducer update state (or perform some logic) based on Action Type.
